...
class Application extends Dacadmin
{
    public $data;
    ...
    function index()
    {
        redirect('dacadmin/application/listapplications', 'refresh');
    }
    ...
    function addapplication()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            //homepage icon
            if ($_FILES['homeicon']['size'] != 0) {
                //$this->callback_image_upload('father_image');
                $imgSize = getimagesize($_FILES["homeicon"]['tmp_name']);
                if ($imgSize[0] > 2048 || $imgSize[1] > 2048) {
                    $message = "Image height or width is larger than 2048px.";
                    $this->session->set_flashdata('error_message', $message);
                    redirect("dacadmin/application/addapplication/");
                ...
                $result = upload_image('homeicon', $target, $thumb, $folder_file);
                if (isset($result['fullname'])) {
                    $homeicon = $result['fullname'];
                }
            }
            $article = array(
                'title' => $title,
